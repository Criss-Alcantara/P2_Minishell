...
void MYHISTORY(char ***argvv, struct command comandos[max_history], int command_counter, char *HOME, char *PWD, time_t comienzo, time_t Final){
  ...
  if(argvv[0][1] == NULL){ /* myhistory sin argumento */
    ...
    if((comandos[atoi(argvv[0][1])].num_commands == 0) ||
       ...
       (atoi (argvv[0][1]) == command_counter)){ /* Evitar llamadas recursivas a si mismo */
      ...
      if(strcmp(comandos[atoi(argvv[0][1])].argvv[0][0], "mytime") == 0){
        ...
        /* Restricciones si el comando a ejecutar del historial es "myhistory" CON argumento */
        /* evitar llamadas recursivas en bucle entre distintos comandos myhistory */
        if((comandos[atoi (argvv[0][1])].argvv[0][1] != NULL) &&
